...
class KendaraanApiController extends Controller
{
    ...
    // --- CRUD Admin/Owner Methods ---

    // Create Vehicle
    public function store(Request $request)
    {
        // Simple authorization check (ideal: use Policy/Gate)
        // Access user via check, assuming middleware 'auth:sanctum' is on route
        $user = $request->user();
        ...
        if ($user->role === 'pemilik_rental') {
             ...
             $id_pemilik = $request->id_pemilik_rental; // Admin must send it
        }
    ...
    public function update(Request $request, $id)
    {
        ...
        // Authorization: Owner can only edit their own
        if ($user->role === 'pemilik_rental') {
             $owner = \App\Modules\Auth\Models\M_PemilikRental::where('id_user', $user->id)->first();
